...
class Face_Recognizer:
    ...
    #从"features_total.csv"读取人脸特征
    def get_face_database(self):
        if os.path.exists("../SDAUAi/data/features_all.csv"):
            path_features_known_csv = "../SDAUAi/data/features_all.csv"
            ...
            for i in range(csv_rd.shape[0]):
                ...
                self.feature_known_list.append(features_someone_arr) #所有人的人脸特征信息，双层列表
                self.name_known_list.append("Person_"+str(i+1))
            print("Faces in Database：", len(self.feature_known_list))
            return 1
        else:
            print("'features_total.csv' not found!")
            return 0

    #计算两个128D向量间的欧式距离
    @staticmethod#类方法直接调用
